...
loadjs.ready("head", function () {
    ...
    fcf_useredit = currentForm = new ew.Form("fcf_useredit", "edit");

    // Add fields
    var fields = ew.vars.tables.cf_user.fields;
    fcf_useredit.addFields([
    ...
    // Validate form
    fcf_useredit.validate = function () {
    ...
    // Form_CustomValidate
    fcf_useredit.customValidate = function(fobj) { // DO NOT CHANGE THIS LINE!
        // Your custom validation code here, return false if invalid.
        return true;
    }

    // Use JavaScript validation or not
    fcf_useredit.validateRequired = <?= Config("CLIENT_VALIDATE") ? "true" : "false" ?>;

    // Dynamic selection lists
    loadjs.done("fcf_useredit");
});
</script>
<script>
loadjs.ready("head", function () {
...
$Page->showMessage();
?>
<form name="fcf_useredit" id="fcf_useredit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl() ?>" method="post">
...
<input type="hidden" data-table="cf_user" data-field="x_empl_id" data-hidden="1" name="o_empl_id" id="o_empl_id" value="<?= HtmlEncode($Page->empl_id->OldValue ?? $Page->empl_id->CurrentValue) ?>">
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fcf_useredit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcf_useredit", "x_last_login", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fcf_useredit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcf_useredit", "x_last_pwd_changed", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<script>
loadjs.ready(["fcf_useredit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcf_useredit", "x_audit_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
...
<?php if ($Page->RowID->Visible) { // RowID ?>
    <div id="r_RowID" class="form-group row">
        <label id="elh_cf_user_RowID" class="<?= $Page->LeftColumnClass ?>"><?= $Page->RowID->caption() ?><?= $Page->RowID->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div <?= $Page->RowID->cellAttributes() ?>>
<span id="el_cf_user_RowID">
<input type="<?= $Page->RowID->getInputTextType() ?>" data-table="cf_user" data-field="x_RowID" name="x_RowID" id="x_RowID" placeholder="<?= HtmlEncode($Page->RowID->getPlaceHolder()) ?>" value="<?= $Page->RowID->EditValue ?>"<?= $Page->RowID->editAttributes() ?> aria-describedby="x_RowID_help">
...
<script>
loadjs.ready(["fcf_useredit", "datetimepicker"], function() {
    ew.createDateTimePicker("fcf_useredit", "x_expired_date", {"ignoreReadonly":true,"useCurrent":false,"format":0});
});
</script>
<?php } ?>
</span>
