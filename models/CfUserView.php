 ...
 * Page class
 */
class CfUserView extends CfUser
{
    use MessagesTrait;
    ...
    public function run()
    {
        ...
        if ($this->isPageRequest()) { // Validate request
            if (($keyValue = Get("empl_id") ?? Route("empl_id")) !== null) {
                ...
                $this->RecKey["empl_id"] = $this->empl_id->QueryStringValue;
            } elseif (Post("empl_id") !== null) {
                $this->empl_id->setFormValue(Post("empl_id"));
                $this->RecKey["empl_id"] = $this->empl_id->FormValue;
            } elseif (IsApi() && ($keyValue = Key(0) ?? Route(2)) !== null) {
                $this->empl_id->setQueryStringValue($keyValue);
                $this->RecKey["empl_id"] = $this->empl_id->QueryStringValue;
            } else {
                $returnUrl = "CfUserList"; // Return to list
            }
            ...
            switch ($this->CurrentAction) {
                case "show": // Get a record to display
                    ...
                    if (!$res) { // Load record based on key
                        ...
                        $returnUrl = "CfUserList"; // No matching record, return to list
                    }
                    break;
            }
        } else {
            $returnUrl = "CfUserList"; // Not page request, return to list
        }
        if ($returnUrl != "") {
